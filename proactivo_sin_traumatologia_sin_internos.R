...
# ---- Variante "sin Internos y sin Traumatología" ----
# Excluye a la vez a los 5 encuestados con cargo Interno/a y a los 3 de la
# sala Traumatología (análisis de sensibilidad combinado; quedan 19
# respuestas: Becado/a = 14, Staff = 5). El id se asigna ANTES de filtrar
# para que se mantenga alineado con `codificacion_tematica.R`, que
# referencia ids del set completo de 27 respuestas.
data <- data_raw %>%
  rename(!!!nombres_cortos) %>%
  mutate(id = row_number(), .before = 1) %>%
  filter(cargo != "Interno/a", sala != "Traumatología")
...
write_csv(data_wide, "data_wide_sin_traumatologia_sin_internos.csv")
...
write_csv(data_long, "data_long_sin_traumatologia_sin_internos.csv")
...
caracteristicas_poblacion %>%
  as_gt() %>%
  gt::gtsave("tabla_caracteristicas_sin_traumatologia_sin_internos.html")

caracteristicas_poblacion %>%
  as_gt() %>%
  gt::gtsave("tabla_caracteristicas_sin_traumatologia_sin_internos.png", vwidth = 900, zoom = 3)
...
grafico_diverging <- ggplot(datos_diverging) +
  ...
  labs(
    title = "Percepción del equipo tratante sobre psiquiatría de enlace (sin Internos ni Traumatología)",
...
ggsave("grafico_diverging_likert_sin_traumatologia_sin_internos.png", grafico_diverging, width = 10, height = 7, dpi = 150)
...
ggsave("grafico_favorable_por_cargo_sin_traumatologia_sin_internos.png", grafico_favorable_cargo, width = 9, height = 7, dpi = 150)
...
ggsave("grafico_indice_vs_experiencia_sin_traumatologia_sin_internos.png", grafico_indice_experiencia, width = 8, height = 6, dpi = 150)
...
ggsave("grafico_indice_vs_interacciones_sin_traumatologia_sin_internos.png", grafico_indice_interacciones, width = 8, height = 6, dpi = 150)
...
ggsave("grafico_correlacion_items_sin_traumatologia_sin_internos.png", grafico_correlacion, width = 9, height = 8, dpi = 150)
...
ggsave("grafico_tematico_sin_traumatologia_sin_internos.png", grafico_tematico, width = 10, height = 6, dpi = 150)
...
ggsave("grafico_palabras_frecuentes_sin_traumatologia_sin_internos.png", grafico_palabras, width = 10, height = 6, dpi = 150)
