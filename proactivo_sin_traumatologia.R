...
# ---- Variante "sin Traumatología" ----
# Excluye a los 3 encuestados de la sala Traumatología (análisis de
# sensibilidad). El id se asigna ANTES de filtrar para que se mantenga
# alineado con `codificacion_tematica.R`, que referencia ids del set
# completo de 27 respuestas.
data <- data_raw %>%
  rename(!!!nombres_cortos) %>%
  mutate(id = row_number(), .before = 1) %>%
  filter(sala != "Traumatología")
...
write_csv(data_wide, "data_wide_sin_traumatologia.csv")
...
write_csv(data_long, "data_long_sin_traumatologia.csv")
...
caracteristicas_poblacion %>%
  as_gt() %>%
  gt::gtsave("tabla_caracteristicas_sin_traumatologia.html")

caracteristicas_poblacion %>%
  as_gt() %>%
  gt::gtsave("tabla_caracteristicas_sin_traumatologia.png", vwidth = 900, zoom = 3)
...
grafico_diverging <- ggplot(datos_diverging) +
  ...
  labs(
    title = "Percepción del equipo tratante sobre psiquiatría de enlace (sin Traumatología)",
...
ggsave("grafico_diverging_likert_sin_traumatologia.png", grafico_diverging, width = 10, height = 7, dpi = 150)
...
ggsave("grafico_favorable_por_cargo_sin_traumatologia.png", grafico_favorable_cargo, width = 9, height = 7, dpi = 150)
...
ggsave("grafico_indice_vs_experiencia_sin_traumatologia.png", grafico_indice_experiencia, width = 8, height = 6, dpi = 150)
...
ggsave("grafico_indice_vs_interacciones_sin_traumatologia.png", grafico_indice_interacciones, width = 8, height = 6, dpi = 150)
...
ggsave("grafico_correlacion_items_sin_traumatologia.png", grafico_correlacion, width = 9, height = 8, dpi = 150)
...
ggsave("grafico_tematico_sin_traumatologia.png", grafico_tematico, width = 10, height = 6, dpi = 150)
...
ggsave("grafico_palabras_frecuentes_sin_traumatologia.png", grafico_palabras, width = 10, height = 6, dpi = 150)
